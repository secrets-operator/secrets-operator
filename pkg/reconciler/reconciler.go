...
func (p Params) CheckNilValues() error {
	if p.Reconciled == nil {
		return fmt.Errorf("reconciled must not be nil")
	}
	if p.UpdateReconciled == nil {
		return fmt.Errorf("UpdateReconciled must not be nil")
	}
	if p.NeedsUpdate == nil {
		return fmt.Errorf("NeedsUpdate must not be nil")
	}
	if p.Expected == nil {
		return fmt.Errorf("expected must not be nil")
	}
	return nil
}

func (p Params) info(message string, keysAndValues ...interface{}) {
	if p.Log != nil {
		(*p.Log).Info(message, keysAndValues)
	}
}

func (p Params) error(err error, message string, keysAndValues ...interface{}) {
	if p.Log != nil {
		(*p.Log).Error(err, message, keysAndValues)
	}
}
...
func ReconcileResource(params Params) error {
	...
	create := func() error {
		params.info("Creating resource", "kind", kind, "namespace", namespace, "name", name)
	...
	if err != nil && apierrors.IsNotFound(err) {
		...
		params.error(err, fmt.Sprintf("Generic GET for %s %s/%s failed with error", kind, namespace, name))
		return fmt.Errorf("failed to get %s %s/%s: %w", kind, namespace, name, err)
	}

	if params.NeedsRecreate != nil && params.NeedsRecreate() {
		params.info("Resource cannot be updated, hence will be deleted and then recreated", "kind", kind, "namespace", namespace, "name", name)
		params.info("Deleting resource", "kind", kind, "namespace", namespace, "name", name)
	...
	if params.NeedsUpdate() {
		params.info("Updating resource", "kind", kind, "namespace", namespace, "name", name)
		...
		if expectedOwners != nil {
			...
			OverrideControllerReference(reconciledMeta, expectedOwners[0])
		}
